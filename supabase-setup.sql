...
-- Users can only CRUD their own scores
...
-- 4. Storage bucket for score files
-- NOTE: Create the 'scores' bucket in the Supabase Dashboard > Storage
-- Set it to PRIVATE, then add these policies:

-- Storage RLS policies (run after creating the bucket):
-- Users can upload to their own folder
-- insert policy: (bucket_id = 'scores') AND (auth.uid()::text = (storage.foldername(name))[1])
-- Users can read their own files
-- select policy: (bucket_id = 'scores') AND (auth.uid()::text = (storage.foldername(name))[1])
-- Users can delete their own files
-- delete policy: (bucket_id = 'scores') AND (auth.uid()::text = (storage.foldername(name))[1])
