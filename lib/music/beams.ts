...
type Element = { kind: "note"; data: NoteData; noteIdx: number } | { kind: "backup" };
  ...
  function flushGroup() {
    if (currentGroup.length >= 2) {
      currentGroup.forEach((ni, gi) => {
        const tag = gi === 0 ? "begin" : gi === currentGroup.length - 1 ? "end" : "continue";
        resultXmls[ni] = resultXmls[ni].replace("</note>", `  <beam number="1">${tag}</beam>\n      </note>`);
      });
    }
    currentGroup = [];
    currentBeat = -1;
